...
const experience = [
  { company: "Mycar", logo: "experience-mycar.png", roles: [{ en: "Product designer", kk: "Önim dizaineri", dates: "06/2025 - now" }, { en: "Graphic designer", kk: "Grafikalyq dizainer", dates: "08/2023 - 06/2025" }] },
  { company: "Kaizen", logo: "experience-kaizen.png", roles: [{ en: "Graphic & Web designer", kk: "Grafikalyq jäne Web dizainer", dates: "01/2023 - 08/2023" }] },
  { company: "Payda", logo: "experience-payda.png", roles: [{ en: "Graphic designer", kk: "Grafikalyq dizainer", dates: "11/2021 - 02/2023" }] },
  { company: "Bugin Holding", logo: "experience-bugin.png", roles: [{ en: "Graphic designer", kk: "Grafikalyq dizainer", dates: "11/2021 - 02/2023" }] },
] as const;
...
function ProjectEntry({ project }: { project: Project }) {
  ...
  return (
    <article className="index-project">
      <div className="index-project-thumb"><ProjectMedia project={project} thumb /></div>
...
function EditorialSections() {
  ...
  return (
    <>
      ...
      <section className="index-editorial" id="experience">
        <p className="index-editorial-label">{lang === "en" ? "Experience" : "Täjiribe"}</p>
        <div className="index-list">{experience.map((item) => <div className="index-experience-row" key={item.company}><span className="index-company-logo"><Image src={`${basePath}/${item.logo}`} alt="" fill sizes="40px" className="object-cover" /></span><p>{item.company}</p><div>{item.roles.map((role) => <div className="index-role" key={role.dates}><p>{role[lang]}</p><p>{role.dates}</p></div>)}</div></div>)}</div>
      </section>
  ...
  return (
    <main className="portfolio-index">
      <aside className="index-profile">
        ...
        <div className="index-intro" id="top"><p className="index-name">{t.name}</p><p>{t.about}</p></div>
